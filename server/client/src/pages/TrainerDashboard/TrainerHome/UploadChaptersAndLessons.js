...
const handleVideoUpload = (chapterIndex, lessonIndex, file) => {
  ...
  try {
    await axios.delete(`https://hilms-mongodb.onrender.com/api/courses/${courseId}/chapters/${chapterId}`);
...
const handleRemoveLesson = async (courseId, chapterId, lessonId, setChapters, chapters) => {
  try {
    await axios.delete(`https://hilms-mongodb.onrender.com/api/courses/${courseId}/chapters/${chapterId}/lessons/${lessonId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post("https://hilms-mongodb.onrender.com/api/courses/upload", formData, {
  ...
  useEffect(() => {
    const fetchChapters = async () => {
      try {
        const response = await axios.get(`https://hilms-mongodb.onrender.com/api/courses/${courseId}/chapters`);
        const formattedChapters = response.data.map((chapter) => ({
          ...
          lessons: chapter.lessons.map((lesson) => ({
            ...
            description: lesson.description || "",
            videoUrl: lesson.videoUrl ? `https://hilms-mongodb.onrender.com/${lesson.videoUrl}` : "", // Ensure full URL
          })),
        }));
